...
def run_test(cmd, is_reference):
    ...
    try:
        ...
        print(e)
        print(
            "%s solution failed correctness check!"
            % ("REFERENCE" if is_reference else "STUDENT")
        )
    return runtimes
...
def pretty_print_with_comparison(test_name, runtimes, perf_threshold, impl_perf_ok):
    ...
    print(
        "%s%s%sPERF?"
        % (" " * 40, "{:<10}".format(AUTHORS[0]), "{:<12}".format(AUTHORS[1]))
    )
    for impl in LIST_OF_IMPLEMENTATIONS:
        ...
        print(
            "{:<40}{:<10}{:<12}{:.2f}  {}".format(
                impl, student_time, ref_time, relative_perf, feedback
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Max number of threads that the task system can use. (%d by default)"
        % TASKSYS_DEFAULT_NUM_THREADS,
    )
    parser.add_argument(
        "-t",
        "--test_names",
        ...
        help="List of tests to run: %s" % ", ".join([x[0] for x in LIST_OF_TESTS]),
    )
    parser.add_argument(
        "-a", "--run_async", action="store_true", help="Run async tests"
    )
    ...
    print(
        "Running task system grading harness... (%d total tests)"
        % len(test_names_and_num_threads)
    )
    ...
    for test_name, num_threads in test_names_and_num_threads:
        ...
        pretty_print_with_comparison(
            test_name, all_runtimes, PERF_THRESHOLD, impl_perf_ok
        )
    ...
    for impl in LIST_OF_IMPLEMENTATIONS:
        final_feedback = (
            "All passed Perf" if impl_perf_ok[impl] else "Perf did not pass all tests"
        )
