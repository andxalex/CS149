...
__global__ void
setlast_kernel(int N, int* result){
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
void exclusive_scan(int* input, int N, int* result)
{
    ...
    // set last element
    setlast_kernel<<<blocks, threadsPerBlock>>>(length, result);
    cudaDeviceSynchronize();
...
__global__ void
findequals_kernel(int* device_input, int length, int* flags){
    
    // Get index
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    // If index is valid, check if current and next elements are equal
    if (index < (length - 1)){
...
__global__ void
createout_kernel(int* flags, int* scan ,int length, int* device_output, int* num_pairs){
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    // if index is valid, populate outputs.
    if (index < length){
        if (flags[index])
            device_output[scan[index]] = index; 
    }

    if (index == 0){
        *num_pairs = scan[length - 1];
    }
}
...
int find_repeats(int* device_input, int length, int* device_output) {
    ...
    // instantiate temp arrays and allocate to device
    int* flags = nullptr;
    int* scan = nullptr;
    ...
    // Then assign 
    createout_kernel<<<blocks, threadsPerBlock>>>(flags, scan, length, device_output, num_pairs);
    cudaDeviceSynchronize();
